...
type discountAttributeR struct {
	DiscountRule           *DiscountRule              `boil:"DiscountRule" json:"DiscountRule" toml:"DiscountRule" yaml:"DiscountRule"`
...
// DiscountRule pointed to by the foreign key.
func (o *DiscountAttribute) DiscountRule(mods ...qm.QueryMod) discountRuleQuery {
	queryMods := []qm.QueryMod{
		qm.Where("`id` = ?", o.DiscountRuleID),
	}

	queryMods = append(queryMods, mods...)

	query := DiscountRules(queryMods...)
	queries.SetFrom(query.Query, "`discount_rule`")
...
// LoadDiscountRule allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for an N-1 relationship.
func (discountAttributeL) LoadDiscountRule(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDiscountAttribute interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.DiscountRuleID) {
			args = append(args, object.DiscountRuleID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.DiscountRuleID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.DiscountRuleID) {
				args = append(args, obj.DiscountRuleID)
			}
	...
	query := NewQuery(
		qm.From(`discount_rule`),
		qm.WhereIn(`discount_rule.id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load DiscountRule")
	}

	var resultSlice []*DiscountRule
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice DiscountRule")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for discount_rule")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for discount_rule")
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if queries.Equal(local.DiscountRuleID, foreign.ID) {
				local.R.DiscountRule = foreign
				if foreign.R == nil {
					foreign.R = &discountRuleR{}
				}
...
// SetDiscountRule of the discountAttribute to the related item.
// Sets o.R.DiscountRule to related.
// Adds o to related.R.DiscountAttributes.
func (o *DiscountAttribute) SetDiscountRule(ctx context.Context, exec boil.ContextExecutor, insert bool, related *DiscountRule) error {
	var err error
	if insert {
	...
	queries.Assign(&o.DiscountRuleID, related.ID)
	if o.R == nil {
		o.R = &discountAttributeR{
			DiscountRule: related,
		}
	} else {
		o.R.DiscountRule = related
	}

	if related.R == nil {
		related.R = &discountRuleR{
...
// RemoveDiscountRule relationship.
// Sets o.R.DiscountRule to nil.
// Removes o from all passed in related items' relationships struct (Optional).
func (o *DiscountAttribute) RemoveDiscountRule(ctx context.Context, exec boil.ContextExecutor, related *DiscountRule) error {
	var err error

	queries.SetScanner(&o.DiscountRuleID, nil)
	if _, err = o.Update(ctx, exec, boil.Whitelist("discount_rule_id")); err != nil {
		return errors.Wrap(err, "failed to update local table")
	}

	if o.R != nil {
		o.R.DiscountRule = nil
	}
	...
	for i, ri := range related.R.DiscountAttributes {
		if queries.Equal(o.DiscountRuleID, ri.DiscountRuleID) {
			continue
		}
...
func (o *DiscountAttribute) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == discountAttributeMapping["id"] {
		goto CacheNoHooks
	}
