...
func testDiscountAttributeToOneDiscountRuleUsingDiscountRule(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local DiscountAttribute
	var foreign DiscountRule

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, discountAttributeDBTypes, true, discountAttributeColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize DiscountAttribute struct: %s", err)
	}
	if err := randomize.Struct(seed, &foreign, discountRuleDBTypes, false, discountRuleColumnsWithDefault...); err != nil {
	...
	queries.Assign(&local.DiscountRuleID, foreign.ID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.DiscountRule().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadDiscountRule(ctx, tx, false, (*[]*DiscountAttribute)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.DiscountRule == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.DiscountRule = nil
	if err = local.L.LoadDiscountRule(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.DiscountRule == nil {
		t.Error("struct should have been eager loaded")
...
func testDiscountAttributeToOneSetOpDiscountRuleUsingDiscountRule(t *testing.T) {
	var err error
	...
	var a DiscountAttribute
	var b, c DiscountRule

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, discountAttributeDBTypes, false, strmangle.SetComplement(discountAttributePrimaryKeyColumns, discountAttributeColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &b, discountRuleDBTypes, false, strmangle.SetComplement(discountRulePrimaryKeyColumns, discountRuleColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, discountRuleDBTypes, false, strmangle.SetComplement(discountRulePrimaryKeyColumns, discountRuleColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*DiscountRule{&b, &c} {
		err = a.SetDiscountRule(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.DiscountRule != x {
		...
		if !queries.Equal(a.DiscountRuleID, x.ID) {
			t.Error("foreign key was wrong value", a.DiscountRuleID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.DiscountRuleID))
		reflect.Indirect(reflect.ValueOf(&a.DiscountRuleID)).Set(zero)
		...
		if !queries.Equal(a.DiscountRuleID, x.ID) {
			t.Error("foreign key was wrong value", a.DiscountRuleID, x.ID)
		}
	}
}

func testDiscountAttributeToOneRemoveOpDiscountRuleUsingDiscountRule(t *testing.T) {
	var err error
	...
	if err = a.SetDiscountRule(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveDiscountRule(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.DiscountRule().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.DiscountRule != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.DiscountRuleID) {
