...
var DiscountMetaAttributeWhere = struct {
	...
	Point:                whereHelperint{field: "`discount_meta_attributes`.`point`"},
}
...
var (
	discountMetaAttributeAllColumns            = []string{"id", "discount_attributes_id", "name", "value", "point"}
	discountMetaAttributeColumnsWithoutDefault = []string{"discount_attributes_id", "name", "value", "point"}
	discountMetaAttributeColumnsWithDefault    = []string{"id"}
...
func (o *DiscountMetaAttribute) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == discountMetaAttributeMapping["id"] {
		goto CacheNoHooks
	}
